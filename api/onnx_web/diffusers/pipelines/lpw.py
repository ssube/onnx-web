###
# From https://github.com/huggingface/diffusers/blob/v0.11.1/examples/community/lpw_stable_diffusion_onnx.py
# Originally by https://github.com/SkyTNT
###
    ...
    if not skip_parsing:
        ...
        prompt_tokens = [
            ...
            for token in pipe.tokenizer(
                prompt, max_length=max_length, truncation=True, return_tensors="np"
            ).input_ids
        ]
...
class OnnxStableDiffusionLongPromptWeightingPipeline(OnnxStableDiffusionPipeline):
    ...
    def prepare_latents(
        self, image, timestep, batch_size, height, width, dtype, generator, latents=None
    ):
        if image is None:
            shape = (
                batch_size,
            ...
            if latents is None:
                latents = (
                    torch.randn(shape, generator=generator, device="cpu")
                    .numpy()
                    .astype(dtype)
                )
            else:
            ...
            latents = (
                torch.from_numpy(latents) * self.scheduler.init_noise_sigma
            ).numpy()
